...
- V1 只支持 EPUB、PDF；不要引入有声书、漫画、OCR、OAuth 或 MFA。
- 按用户新增要求，邮件仅用于找回邮箱验证、密码重置与主人 SMTP 配置测试；不扩展邮件阅读或群发功能。
...
- 密码复杂度由主人在安全设置配置；注册、初始化、改密和邮件重置必须执行同一服务端规则，页面提前显示当前要求与未满足项。规则变更不阻止已有密码登录。
