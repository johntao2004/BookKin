  ...
  return (
    <AuthFrame title="先换一把自己的钥匙" description="管理员创建的是临时密码。完成修改后，才能进入藏书与阅读数据。">
      <Stack component="form" spacing={2.5} onSubmit={submit}>
        <TextField label="当前临时密码" type="password" autoComplete="current-password" value={currentPassword} onChange={(event) => setCurrentPassword(event.target.value)} required autoFocus />
        <TextField label="新密码" type="password" autoComplete="new-password" value={newPassword} onChange={(event) => setNewPassword(event.target.value)} required slotProps={{ htmlInput: { minLength: 12 } }} />
        <LinearProgress variant="determinate" value={strength} color={strength >= 75 ? "success" : "primary"} sx={{ height: 4, borderRadius: 2 }} />
        <TextField label="确认新密码" type="password" autoComplete="new-password" value={confirmPassword} onChange={(event) => setConfirmPassword(event.target.value)} required />
